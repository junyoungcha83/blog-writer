...
- 들여쓰기·글꼴 크기 같은 서식은 앱이 알아서 붙인다. 문장 앞에 공백을 넣거나 HTML 태그를 쓰지 않는다.
...
- **그림 자리**: 사람이 직접 사진·그림·도표를 만들어 넣을 자리를 본문에 ${len.sections >= 5 ? '3~4' : '2~3'}곳 잡는다.
  그 문단 <b>바로 뒤</b>에 \`[[img:1]]\` \`[[img:2]]\` 처럼 자리표시자를 빈 줄로 둘러싼 독립된 줄에 넣고,
  같은 번호를 images 배열에 순서대로(1부터) 적는다.
  caption 은 그 문단의 내용을 짚어 주는 한 문장으로, 그림 아래 설명글로 그대로 쓸 수 있게 쓴다
  (예: "부동산 대출 조이기가 다음 달부터 시행되면서 서민들의 고민이 깊어지고 있습니다").
  캡션도 위 어투를 따르고, 본문에 없는 사실을 새로 만들지 않는다.
  idea 는 어떤 그림·사진·도표를 만들면 좋을지 사람이 읽고 바로 작업할 수 있게 한두 문장으로 적는다.
  이미 [[viz:N]] 차트로 보여 주는 내용과 겹치는 그림은 잡지 않는다. 첫 문단 앞과 글 맨 끝에는 두지 않는다.
${risky
...
출력 JSON 스키마:
{
  ...
  "images": [ { "n": 1, "caption": "그림 아래에 들어갈 설명 한 문장", "idea": "어떤 그림·사진·도표를 만들면 좋은지" } ],
